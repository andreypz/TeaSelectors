...
if __name__ == "__main__":
  ...
  for n in ["lep1_pt", "lep2_pt", "gamma_pt",
            ...
            "Mllg_full", "Mllg_higg", "Mllg_zed", "Mll_APZ_JPsi",
    ...
    print "Plotting", n
    m = 0
    for d in ['InAPZ', 'AfterCuts', 'InJpsi']:
      h[d] = f.Get(d+'/'+n+'_'+d)
      if h[d] == None:
      ...
      norm = h['InAPZ'].Integral()
      
      if h[d].Integral()!=0:
        h[d].Scale(norm/h[d].Integral())
      
      if n in ["Mllg_full","gamma_pt","Mjj_full","Mllgj","Mgj_full","pt_ratio_dilep_gamma"]:
        handleOverflowBins(h[d])
            
      h[d].Draw(opt)
      m = max(h[d].GetMaximum(), m)
      h['InAPZ'].SetMaximum(1.1*m)
